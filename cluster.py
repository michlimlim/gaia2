...
class Cluster:
    ...
    def go(self):
        ...
        for it in range(iterations):
            ...
            for i in range(m):
                # Compute update
                print("[cluster: {:2d}] iter = {:d}".format(
                    self.get_id(), iter))
                theta, cost = compute_local_theta(
                    m, self.X, self.y, theta, cost, self.machine_speed/10.0)
                # Global aggregation
                if iter > 0 and (iter % self.delta) == 0:
                    ...
                    if significance > self.significance_threshold(iter):
                        print("[cluster: {:2d}] global update step  = {:d} sig= {:02f} PAST THRESHOLD!".format(
                            self.get_id(), iter//self.delta, significance))
                        agg = self.global_aggregate(theta)
                        if agg.any() != global_theta.any():
                            theta = agg
                            global_theta = theta
                    else:
                        print("[cluster: {:2d}] global update step  = {:d} sig= {:02f} Skip Aggregation".format(
                            self.get_id(), iter//self.delta, significance))
                        self.send_skip()
                iter += 1
